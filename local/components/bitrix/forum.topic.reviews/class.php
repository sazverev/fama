...
final class ForumTopicReviewsComponent extends CBitrixComponent implements Main\Engine\Contract\Controllerable
{
	...
	protected function checkPostActions()
	{
		...
		elseif (mb_strlen($data["REVIEW_TEXT"]) < 1)
		{
			...
			!$this->checkCaptchta($data))
		{
			//$result->addError(new Main\Error(Loc::getMessage("POSTM_CAPTCHA"), "bad captcha"));
			$result->addError(new Main\Error(print_r($data, true)));
		}
	...
	private function checkCaptchta(&$data)
	{
		if(!empty($data['g-recaptcha-response']))
		{
			return true;
			/*
			$secretKey = "6LfbXLMlAAAAACySfJ13AoU3N2hyZhPeCx6hD81c";
			$response = $data["g-recaptcha-response"];
			$remoteIp = $_SERVER['REMOTE_ADDR'];
			$reCaptchaValidationUrl = file_get_contents("https://www.google.com/recaptcha/api/siteverify?secret=$secretKey&response=$response&remoteip=$remoteIp");
			$result = json_decode($reCaptchaValidationUrl);

			$data['checkCaptchta'] = $result;

			return $result->success;
			*/
		}
		else		
		if (mb_strlen($data["captcha_code"]) > 0)
		{
			include_once($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/main/classes/general/captcha.php");
			$captchaPass = Main\Config\Option::get("main", "captcha_password", "");

			$cpt = new CCaptcha();
			return $cpt->CheckCodeCrypt($data["captcha_word"], $data["captcha_code"], $captchaPass);
		}
		else{
			return false;
		}
