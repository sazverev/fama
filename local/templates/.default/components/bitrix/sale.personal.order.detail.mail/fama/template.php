	...
	<table class="bx_order_list_table_order">
		...
		<tr>
			...
			foreach($arParams["CUSTOM_SELECT_PROPS"] as $headerId):
				...
				if(mb_strpos($headerId, 'PROPERTY_') === false) {
		...
		foreach($arResult["BASKET"] as $prod):
			...
			foreach($arParams["CUSTOM_SELECT_PROPS"] as $headerId):
				...
				elseif($headerId == "PRICE_FORMATED"):
					...
					$value = $prod[(mb_strpos($headerId, 'PROPERTY_') === 0? $headerId.'_VALUE' : $headerId)];
					
					if(!empty($value))
					{
						//$strOptions .= $headerName .": ". $value . "<br/>";
						$strOptions .= "<tr><td class='custom_t1'>". $headerName .": </td><td class='custom_t2'>". $value . "</td></tr>";
					}
			...
			?><tr><td colspan="4"><?
				...
				echo "<table class='bx_ordercart_order_sum'><tbody>".$strOptions."</tbody></table>";
