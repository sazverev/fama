						...
						<?foreach($arResult['SECTIONS'] as $si => $arSection):?>
							...
							<?foreach($arSection['ITEMS'] as $i => $arItem):?>
								...
								<tr class="item" id="<?=$this->GetEditAreaId($arItem['ID'])?>">
									...
									<td class="brands" style="width:30%;">
									<?if (!empty($arItem["PROPERTIES"]["BRANDS"]["VALUE"])):?>
										<?foreach($arItem["PROPERTIES"]["BRANDS"]["VALUE"] as $photo):?>
											<img width="32%" src="<?=CFile::GetPath($photo)?>" />
										<?endforeach?>
									<?endif?>
									</td>
