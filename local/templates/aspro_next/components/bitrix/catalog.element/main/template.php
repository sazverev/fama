							...
							<?if('L' == $arResult['PROPERTIES'][$propID]['PROPERTY_TYPE'] && 'C' == $arResult['PROPERTIES'][$propID]['LIST_TYPE']){
								...
								<select name="<? echo $arParams['PRODUCT_PROPS_VARIABLE']; ?>[<? echo $propID; ?>]">
									<?foreach($propInfo['VALUES'] as $valueID => $value){?>
										...
										<? $selected="";
											if($valueID == $propInfo['SELECTED'])  $selected="selected";
										 ?>
										<option value="<? echo $valueID; ?>" <? echo ( $selected ); ?>><? echo $value; ?></option>
									<?}?>
								</select>
							<?}?>
						</td>
...
$current_offer=0;
if(!empty($arParams["OFFER_ID"]) && $arResult['OFFERS'])
{
	foreach($arResult['OFFERS'] as $key=>$offer)
	{
...
if($current_offer)
{
    echo "<pre>"; print_r($current_offer); echo "</pre>";
}
*/
...
else
{
	/*
$arViewedData = array(
	'PRODUCT_ID' => $current_offer['LINK_ELEMENT_ID'],
	'IBLOCK_ID' => $current_offer['IBLOCK_ID'],
	'NAME' => $current_offer['NAME'],
	'DETAIL_PAGE_URL' => $arResult['DETAIL_PAGE_URL']."/".$current_offer['ID']."/",
	'PICTURE_ID' => $arResult['PREVIEW_PICTURE'] ? $arResult['PREVIEW_PICTURE']['ID'] : ($arFirstPhoto ? $arFirstPhoto['ID'] : false),
	'MIN_PRICE' => $arItemPrices,
	'CAN_BUY' => $arResult['CAN_BUY'] ? 'Y' : 'N',
	'IS_OFFER' => 'Y',
...
console.log("<?=$current_offer['ID']?>")
console.log("<?=$current_offer['NAME']?>")
	//setViewedProduct(<?=$current_offer['ID']?>, <?=CUtil::PhpToJSObject($arViewedData, false)?>);
</script>
				...
				<div class="top_info">
					...
					<?if(strlen($arResult["PREVIEW_TEXT"])):?>
						...
						<?if(strlen($arResult["DETAIL_TEXT"])):?>
							<div class="more_block icons_fa color_link"><span><a href="#nav-tabs" data-toggle="tab">Подробнее</a></span></div>
			...
			<?$frame = $this->createFrame()->begin();?>
				...
				<div class="buy_block ">
					<?if($arResult["OFFERS"] && $showCustomOffer){?>
						<div class="sku_props <?=$arParams["OFFER_ID"];?>">
							<?if (!empty($arResult['OFFERS_PROP'])){?>
								<div class="bx_catalog_item_scu wrapper_sku" id="<? echo $arItemIDs["ALL_ITEM_IDS"]['PROP_DIV']; ?>">
									<?foreach ($arSkuTemplate as $code => $strTemplate){
										...
										if($code == "COLOR_REF" && strpos($strTemplate, "myDiv2") === false){

											$strTemplate = str_replace('</div></div></div>','</div></div>', $strTemplate);
											$strTemplate = str_replace('class="bx_item_detail_size"','style="display:none"', $strTemplate);
										}

										$strTemplate = str_replace('href="##"','', $strTemplate);
										//$strTemplate = str_replace('href="#"',"", $strTemplate);
										echo str_replace('#ITEM#_prop_', $arItemIDs["ALL_ITEM_IDS"]['PROP'], $strTemplate);
									}?>
									<div class="mobkred">

									<button style="display: block;padding: 15px 12px;border-radius: 50%;display: block;float:left;font-weight: bold;" class="kred" onclick="chpok('textt')">?</button>
									<p id="textt" class="ccc" style="left:0;width:100%;bottom:115px">
									  При оформлении заказа в способе оплаты необходимо выбрать «В кредит». Сумма заказа, доступного для покупки в кредит - от 3 000 до 500 000 рублей, без первоначального взноса. Решение банка - в течение 2 - 5 минут без визита в офис и лишней траты времени. Кредитование предоставляется АО «Тинькофф Банк». <a target="_blank" href=" /katalogipdf/Инструкция Тинькофф Кредит.pdf">Подробности</a> и условия кредитования уточняйте у менеджеров интернет-магазина по номеру 8(800)555-3379 и в наших фирменных шоу-румах
									</p>
									<p style="margin-bottom: 12px;margin-top:15px;">В кредит</p>
									</div>
								</div>
							<?}?>
...
function()
{
	$("#myDiv2").on('click',function()
	{
		...
		$( init );
			...
			$('.active #myDiv1').css('display', 'none');
	...
	$(".close1").on( "click", function() {
		...
		// Убираем фрагмент из URL (в данном случае, #zatemnenie)
		var newUrl = currentUrl.replace('#zatemnenie', '#');

		//переходим по url без zatemnenie
...
$( init );
	...
	$('.active #myDiv1').css('display', '');
		//$('#myDiv2').append($('.active #myDiv1'));
         $(".active #myDiv1").clone().appendTo("#myDiv2");
		//$('#myDiv2').append($('.active #myDiv1'));
		//$('.active #myDiv3').append($('#myDiv1').cloneNode(true));
$('.active #myDiv1').css('display', 'none');

};
return document.getElementById('myDiv2').innerHtml = newContent;
});*/
...
$('#btn_calc').click(function(){
	...
	let text = "Обратитесь к консультанту";

	let q = Number($('.calc .ik_select_active').attr('data-value'));
	if(q!==0)
	{
		let s = Number($('.calc input[name=s]').val());
		let result = (s+0.0)  / (q+0.0);

		if (!isNaN(result) && isFinite(result)) 
		{
			text = "Ожидаемый расход:  <span>" + result.toFixed(1) + "</span> л.";
					...
					<?if(!$arResult["OFFERS"]):?>
						...
						<div class="counter_wrapp">
							...
							<div id="<? echo $arItemIDs["ALL_ITEM_IDS"]['BASKET_ACTIONS']; ?>" class="button_block <?=(($arAddToBasketData["ACTION"] == "ORDER" /*&& !$arResult["CAN_BUY"]*/) || !$arAddToBasketData["CAN_BUY"] || !$arAddToBasketData["OPTIONS"]["USE_PRODUCT_QUANTITY_DETAIL"] || ($arAddToBasketData["ACTION"] == "SUBSCRIBE" && $arResult["CATALOG_SUBSCRIBE"] == "Y")  ? "wide" : "");?>">
								...
								<? if((int)$arResult["PRODUCT"]["QUANTITY"]) echo $arAddToBasketData["HTML"]; else { ?>
									<script data-b24-form="click/78/l51ke5" data-skip-moving="true">
									(function(w,d,u){
									var s=d.createElement('script');s.async=true;s.src=u+'?'+(Date.now()/180000|0);
									var h=d.getElementsByTagName('script')[0];h.parentNode.insertBefore(s,h);
									})(window,document,'https://cdn-ru.bitrix24.ru/b11298180/crm/form/loader_78.js');
									</script>
									<button class="btn-lg w_icons to-order btn btn-default white grey transition_bg transparent ">Под заказ</button>
								<? } ?>
...
<button class="kred" onclick="chpok('text')">В кредит</button>
<p id="text" class="ccc">
  При оформлении заказа в способе оплаты необходимо выбрать «В кредит». Сумма заказа, доступного для покупки в кредит - от 3 000 до 500 000 рублей, без первоначального взноса. Решение банка - в течение 2 - 5 минут без визита в офис и лишней траты времени. Кредитование предоставляется АО «Тинькофф Банк». <a target="_blank" href=" /katalogipdf/Инструкция Тинькофф Кредит.pdf">Подробности</a> и условия кредитования уточняйте у менеджеров интернет-магазина по номеру 8(800)555-3379 и в наших фирменных шоу-румах
</p>
...
/*if($USER->IsAdmin() )
{
	echo "r=".$arResult['PROPERTIES']['RASHOD1_INT']['VALUE'];
}
*/
?>

			<? if((!empty($arResult['PROPERTIES']['RASHOD1_INT']['VALUE']) || !empty($arResult['PROPERTIES']['RASHOD2_INT']['VALUE'])  ))
					{?>
						<div class="calc row">
							<div class="col-12 md-12">
								<label>Калькулятор ремонта</label>
								<div class="inline">
									<div class="w50" style="vertical-align: top;">
										<span class="show_class bx_item_section_name"><span>Площадь, кв. м</span></span>
										<input type="text" class="form-control" style="border-radius: 20px;  width: 120px;" name="s" value="20"  />
									</div>
									<div class="w50">
										<span class="show_class bx_item_section_name"><span>Количество слоев</span></span>
										<select class="form-control" id="c" name="c">
											<option value=""> - выберите -</option>
											<option value="<?=$arResult['PROPERTIES']['RASHOD1_INT']['VALUE'] ?>">1 слой</option>
											<option value="<?=$arResult['PROPERTIES']['RASHOD2_INT']['VALUE'] ?>">2 слоя</option>
										</select>
									</div>
								...
								<div class="inline">
									<div class="w50">
										<button class="calc" id="btn_calc">Рассчитать</button>
									</div>
									<div class="w50">
										<div id="calc_rez"></div>
									</div>
...
<div class="tabs_section">
	...
	if($arResult["DISPLAY_PROPERTIES"]){
		foreach($arResult["DISPLAY_PROPERTIES"] as $arProp){
			if(!in_array($arProp["CODE"], array("SERVICES", "BRAND", "HIT", "RECOMMEND", "NEW", "STOCK", "VIDEO", "VIDEO_YOUTUBE", "TEXT_VIDEO", "CML2_ARTICLE"))){
			...
			<?if($arVideo):?>
				...
				<?endif;?></div>
					...
					<div class="video_text">
						<?=$arResult["PROPERTIES"]["TEXT_VIDEO"]["VALUE"]?>
					</div>
				</div>
			<?endif;?>
			<?if($arParams["USE_REVIEW"] == "Y"):?>
				...
				<div class="tab-pane<?=(!($iTab++) ? ' acive' : '')?>" id="ask">
					...
					<div class="row">
						...
						<div class="col-md-9 form_block">
							<!--<div id="ask_block1"></div>-->
							<script data-b24-form="inline/18/630an2" data-skip-moving="true">
							(function(w,d,u){
							var s=d.createElement('script');s.async=true;s.src=u+'?'+(Date.now()/180000|0);
							var h=d.getElementsByTagName('script')[0];h.parentNode.insertBefore(s,h);
							})(window,document,'https://cdn-ru.bitrix24.ru/b11298180/crm/form/loader_18.js');
							</script>
						</div>
					</div>
				</div>
