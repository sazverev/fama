...
<? $col32=0;?>
		<?foreach($arResult["ITEMS"] as $arItem){?>
			...
			<div class="catalog_glav catalog_item_wrapp col-lg-<?=$col;?> col-md-4 col-sm-<?=floor(12 / round($arParams['LINE_ELEMENT_COUNT'] / 2))?> item_block" data-col="<?=$col;?>"<?if ($col32!=3){?>style="margin: 0 1% 20px 0;"<?} else {?>style="margin: 0 0px 20px 0;"<?}?>>
			<? $col32=$col32+1;
				echo "$col32";			?>
				...
				<div class="catalog_item item_wrap main_item_wrapper" id="<?=$this->GetEditAreaId($arItem['ID']);?>_<?=$arParams["FILTER_HIT_PROP"]?>" style="background: 0;">
					<div class="catalog_glav1 inner_wrap">
						
						<div class="item_info <?=$arParams["TYPE_SKU"]?>" style="width: 58%; float: left; height: 120px;text-align: left;">
...
">
								...
								<?}?>
							</div><div class="item-title" style="margin:5px 0px 5px 0px;">
								<a href="<?=$arItem["DETAIL_PAGE_URL"]?>" class="dark_link"><span style="
...
"><?=$elementName;?></span></a>
							...
							<?endif;*/?>
									<div class="preview_text dotdot" style="font-size:14px;font-family: Gotham Pro;
    font-size: 12px;
    line-height: 16px;"><?=TruncateText($arItem["PREVIEW_TEXT"], 75);?></div>
				...
				<!--div class="sa_block">
							...
							<?if($arParams["SHOW_DISCOUNT_TIME"]=="Y"){?>
								...
								<?endif;?>
							<?}?>
							<div class="<?=($arItem["OFFERS"] && $arItem['OFFERS_PROP'] ? 'has_offer_prop' : '');?> inner_content js_offers__<?=$arItem['ID'];?>_<?=$arParams["FILTER_HIT_PROP"]?>">
							<div class="sku_props">
								<?if($arItem["OFFERS"]){?>
						...
						<div class="image_wrapper_block" style="
    float: right;
    width: 42%;
">
