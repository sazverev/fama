...
<?if($itemsCnt):?>
	...
	<div class="right_block1 clearfix catalog <?=strtolower($arTheme["FILTER_VIEW"]["VALUE"]);?>" id="right_block_ajax">
		...
		<?if($arTheme["FILTER_VIEW"]["VALUE"]=="HORIZONTAL" || $arTheme["FILTER_VIEW"]["VALUE"]=="COMPACT"){?>
			<div class="<?=($arTheme["FILTER_VIEW"]["VALUE"]=="HORIZONTAL" ? 'filter_horizontal' : '');?> swipeignore">
				<?include_once(__DIR__."/../filter.php")?>
			</div>
		<?}/*else{?>
...
<?endif;?>
			<?if(!$arSeoItem):?>
				<?if($arParams["SHOW_SECTION_DESC"] != 'N' && strpos($_SERVER['REQUEST_URI'], 'PAGEN') === false && $arRegion["PROPERTY_MAIN_DOMAIN_VALUE"] == 'famaprofi.ru'):?>
					<?ob_start();?>
					...
					<?elseif($posSectionDescr=="TOP"):?>
						<?if ($arSection[$arParams["SECTION_PREVIEW_PROPERTY"]]):?>
							<div class="group_description_block top" style="
    padding: 20px;
    background: #fff;
    margin-bottom: 10px;
">
					...
					?>
				<?endif;?>
				<?$APPLICATION->ShowViewContent('top_desc_seo');?>
			<?endif;?>
<?if($itemsCnt):?>
...
?>
			<?if( $isAjax=="N"){?>
				<?if(empty($sotbitSeoMetaBottomDesc) && !$arSeoItem):?>

					<?if(strpos($APPLICATION->GetCurDir(), "/filter/") === false && $arParams["SHOW_SECTION_DESC"] != 'N' && strpos($_SERVER['REQUEST_URI'], 'PAGEN') === false && $arRegion["PROPERTY_MAIN_DOMAIN_VALUE"] == 'famaprofi.ru'):?>
						<?ob_start();?>
						<?if($posSectionDescr=="BOTH"):?>
						...
						?>
					<?endif;?>
					<?$APPLICATION->ShowViewContent('bottom_desc_seo');?>
				<?else:?>
					<?ob_start();?>
