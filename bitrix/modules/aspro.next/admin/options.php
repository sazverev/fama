				...
				foreach($moduleClass::$arParametrsList as $blockCode => $arBlock){
					...
					foreach($arBlock["OPTIONS"] as $optionCode => $arOption){
						if($arOption['TYPE'] === 'array'){
							...
							if($optionCode == "CUSTOM_FONT")
							{
								$path = \Bitrix\Main\Application::getDocumentRoot().'/bitrix/components/aspro/theme.next/css/user_font_'.$optionsSiteID.'.css';
								$content = '';
								if($newVal)
								{
										...
										<td colspan="2">
											<table class="aspro-admin-item-table">
												...
												<tr class="aspro-admin-item<?=(!$itemKey ? ' first' : '')?><?=($itemKey == $itemsKeysCount - 1 ? ' last' : '')?><?=($itemKey == $cnt ? ' new' : '')?>" data-itemkey="<?=$itemKey?>" style="text-align:center;"><?
													...
													?>
												<tr style="text-align:center;">
													<td colspan="<?=count($_arParameters)?>"><a href="javascript:;" class="adm-btn adm-btn-save adm-btn-add" title="<?=GetMessage('PRIME_OPTIONS_ADD_BUTTON_TITLE')?>"><?=GetMessage('OPTIONS_ADD_BUTTON_TITLE')?></a></td>
												</tr>
											</table>
										</td>
									</tr><?
