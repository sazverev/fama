...
if($RIGHT >= "R"){
	...
	<?else:?>
		<div class="adm-info-message"><?=GetMessage("ASPRO_NEXT_MODULE_DEVELOP_INFO");?></div>
		<br>
		<br>
		<?$tabControl->Begin();?>
										...
										<td colspan="2">
											<table class="aspro-admin-item-table">
												...
												<tr class="aspro-admin-item<?=(!$itemKey ? ' first' : '')?><?=($itemKey == $itemsKeysCount - 1 ? ' last' : '')?><?=($itemKey == $cnt ? ' new' : '')?>" data-itemkey="<?=$itemKey?>" style="text-align:center;"><?
													...
													?>
												<tr style="text-align:center;">
													<td><a href="javascript:;" class="adm-btn adm-btn-save adm-btn-add" title="<?=GetMessage('OPTIONS_ADD_BUTTON_TITLE')?>"><?=GetMessage('OPTIONS_ADD_BUTTON_TITLE')?></a></td>
												</tr>
											</table>
										</td>
									</tr><?
...
{
	echo CAdminMessage::ShowMessage(GetMessage('ASPRO_NEXT_NO_RIGHTS_FOR_VIEWING'));
}?>
