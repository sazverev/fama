...
while ($c = $conds->fetch()) {
    if (
            !empty($chpu["CONDITION_ID"])
            && $chpu["CONDITION_ID"] == $c['ID']
    ) {
...
if ($request->getRequestMethod() === "POST" && ($request->get('save') || $request->get('apply')) && $rights == "W" && check_bitrix_sessid()) {
    //arrFields main chpu
    $arNewUrlEncode = ['REAL_URL'=>$request->get('REAL_URL'), 'NEW_URL'=>$request->get('NEW_URL')];
    foreach ($arNewUrlEncode as $keyUrl=>$url){
        $match = [];
        $url = Encoding::convertEncoding($url, LANG_CHARSET, "utf-8", $error);
        preg_match_all('/[\p{Cyrillic}]+/iu', $url, $match);
        foreach ($match[0] as $i => $m) {
            $url = str_replace($m, rawurlencode($m), $url);
        }
        $url = str_replace(' ', '%20', $url);
        $arNewUrlEncode[$keyUrl] = $url;
    }
    $arFields = [
        ...
        "REAL_URL" =>$arNewUrlEncode['REAL_URL'] ?? $request->get('REAL_URL'),
        "NEW_URL" => $arNewUrlEncode['NEW_URL'] ?? $request->get('NEW_URL'),
    ...
    if ($request->get('SEOMETA_DATA')) {
        ...
        $chpuSeoData['BITRIX_URL'] = $arNewUrlEncode['REAL_URL'] ?? $request->get('REAL_URL');
...
<tr id="CATEGORY_ID">
    ...
    <td width="60%">
<?= SelectBoxFromArray('CATEGORY_ID', $AllSections, $chpu['CATEGORY_ID'],'',false,'','style="min-width:350px"');?>
</td>
</tr><?
...
<tr class="adm-detail-valign-top">
    ...
    <td width="60%">
        ...
        $chpu['REAL_URL'] = rawurldecode($chpu['REAL_URL']);
        $chpu['REAL_URL'] = Encoding::convertEncoding($chpu['REAL_URL'], "utf-8", LANG_CHARSET, $error);
        ?>
...
<tr class="adm-detail-valign-top">
    ...
    <td width="60%">
        ...
        $chpu['NEW_URL'] = rawurldecode($chpu['NEW_URL']);
        $chpu['NEW_URL'] = Encoding::convertEncoding($chpu['NEW_URL'], "utf-8", LANG_CHARSET, $error);
        ?>
