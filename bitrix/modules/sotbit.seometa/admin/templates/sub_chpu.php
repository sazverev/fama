...
while ($arRes = $rsData->Fetch()) {
    $arRes["T"] = "P";
    $arRes['REAL_URL'] = str_replace(' ', '%20', Encoding::convertEncodingToCurrent(rawurldecode($arRes['REAL_URL'])));
    $arResult[] = $arRes;
}
...
while ($arRes = $rsData->NavNext(true, "")) {
    ...
    if(is_array($arSites = unserialize($condition['SITES'])) && count($arSites) == 1) {
        $siteUrl = $SitesAll[current($arSites)]['SERVER_NAME'];
