			...
			if (!$boolValueError)
			{
				...
				switch ($arControl['FIELD_TYPE'])
				{
					...
					case 'datetime':
						...
						$boolValueError = SMGlobalCondCtrl::ConvertInt2DateTime($arOneCondition['value'], $strFormat, $intOffset);
						break;
					default:
						$boolValueError = true;
						break;
			...
			if (!$boolError)
			{
				...
				switch ($arControl['FIELD_TYPE'])
				{
					...
					case 'datetime':
						...
						$boolError = SMGlobalCondCtrl::ConvertDateTime2Int($arOneCondition['value'], $strFormat, $intOffset);
						break;
					default:
						$boolError = true;
						break;
	...
	public static function Validate($arOneCondition, $arParams, $arControl, $boolShow)
	{
		...
		if (!$boolError)
		{
			...
			else
			{
				...
				{
					case 'element':
						$rsItems = CIBlockElement::GetList(array(), array('ID' => $arOneCondition['value']), array('ID'), false, array('ID'));
						if ($boolMulti)
						{
							$arCheckResult = array();
							while ($arItem = $rsItems->Fetch())
					...
					else
					{
						...
						switch ($arOneAtom['ATOM']['FIELD_TYPE'])
						{
							...
							case 'datetime':
								...
								$boolAtomError = SMGlobalCondCtrl::ConvertInt2DateTime($arOneCondition[$strID], $strFormat, $intOffset);
								break;
							default:
								$boolAtomError = true;
						}
					...
					if (!$boolAtomError)
					{
						switch ($arOneAtom['ATOM']['FIELD_TYPE'])
						{
							...
							case 'datetime':
								...
								$boolAtomError = SMGlobalCondCtrl::ConvertDateTime2Int($arOneCondition[$strName], $strFormat, $intOffset);
								break;
							default:
								$boolAtomError = true;
						}
				...
				foreach ($arControl['ATOMS'] as &$arOneAtom)
				{
					...
					switch ($arOneAtom['ATOM']['VALIDATE'])
					{
						case 'list':
							...
							break;
						case 'element':
							$rsItems = CIBlockElement::GetList(array(), array('ID' => $arValues[$strID]), array('ID', 'NAME'), false, array('ID', 'NAME'));
							if (is_array($arValues[$strID]))
							{
								$arCheckResult = array();
								while ($arItem = $rsItems->Fetch())
				...
				foreach ($arControl['ATOMS'] as &$arOneAtom)
				{
					...
					switch ($arOneAtom['ATOM']['VALIDATE'])
					{
						case 'list':
							...
							break;
						case 'element':
							$rsItems = CIBlockElement::GetList(array(), array('ID' => $arValues[$strID]), array('ID'), false, array('ID'));
							if (is_array($arValues[$strID]))
							{
								$arCheckResult = array();
								while ($arItem = $rsItems->Fetch())
...
class SMGlobalCondCtrlComplex extends SMGlobalCondCtrl
{
	public static function GetControlDescr()
	{
		$strClassName = get_called_class();
...
class SMGlobalCondCtrlGroup extends SMGlobalCondCtrl
{
	public static function GetControlDescr()
	{
		$strClassName = get_called_class();
...
class SMCatalogCondCtrl extends SMGlobalCondCtrl
{
}

class SMCatalogCondCtrlComplex extends SMGlobalCondCtrlComplex
{
}

class SMCatalogCondCtrlGroup extends SMGlobalCondCtrlGroup
{
}

class SMCatalogCondCtrlIBlockFields extends SMCatalogCondCtrlComplex
{
	/**
	 * @return string|array
	 */
	...
	public static function GetControls($strControlID = false)
	{
		...
		if(Loader::IncludeModule("catalog"))
		{
            ...
            while($arPriceType = $dbPriceType->Fetch())
            {
				...
				$arControlListPrice=array(
					'CondIBMaxPrice'.$arPriceType['NAME'] => array(
						'ID' => 'CondIBMaxPrice'.$arPriceType['NAME'],
						'FIELD' => 'MaxPrice'.$arPriceType['NAME'],
						'FIELD_TYPE' => 'int',
						'LABEL' => Loc::getMessage('BT_MOD_CATALOG_COND_CMP_MAX_PRICE').' '.$name,
						'PREFIX' => Loc::getMessage('BT_MOD_CATALOG_COND_CMP_MAX_PRICE_PREFIX').' '.$name,
					...
					'CondIBMinPrice'.$arPriceType['NAME'] => array(
						'ID' => 'CondIBMinPrice'.$arPriceType['NAME'],
						'FIELD' => 'MinPrice'.$arPriceType['NAME'],
						'FIELD_TYPE' => 'int',
						'LABEL' => Loc::getMessage('BT_MOD_CATALOG_COND_CMP_MIN_PRICE').' '.$name,
						'PREFIX' => Loc::getMessage('BT_MOD_CATALOG_COND_CMP_MIN_PRICE_PREFIX').' '.$name,
						'LOGIC' => static::GetLogic(array(BT_COND_LOGIC_EQ/*, BT_COND_LOGIC_NOT_EQ, BT_COND_LOGIC_GR, BT_COND_LOGIC_LS, BT_COND_LOGIC_EGR, BT_COND_LOGIC_ELS*/)),
						'JS_VALUE' => array(
							'type' => 'input'
						),
					...
					'CondIBMaxFilterPrice'.$arPriceType['NAME'] => array(
						'ID' => 'CondIBMaxFilterPrice'.$arPriceType['NAME'],
						'FIELD' => 'MaxFilterPrice'.$arPriceType['NAME'],
						'FIELD_TYPE' => 'int',
						'LABEL' => Loc::getMessage('BT_MOD_CATALOG_COND_CMP_MAX_FILTER_PRICE').' '.$name,
						'PREFIX' => Loc::getMessage('BT_MOD_CATALOG_COND_CMP_MAX_FILTER_PRICE_PREFIX').' '.$name,
						'LOGIC' => static::GetLogic(array(BT_COND_LOGIC_EQ/*, BT_COND_LOGIC_NOT_EQ, BT_COND_LOGIC_GR, BT_COND_LOGIC_LS, BT_COND_LOGIC_EGR, BT_COND_LOGIC_ELS*/)),
						'JS_VALUE' => array(
							'type' => 'input'
						),
					...
					'CondIBMinFilterPrice'.$arPriceType['NAME'] => array(
						'ID' => 'CondIBMinFilterPrice'.$arPriceType['NAME'],
						'FIELD' => 'MinFilterPrice'.$arPriceType['NAME'],
						'FIELD_TYPE' => 'int',
						'LABEL' => Loc::getMessage('BT_MOD_CATALOG_COND_CMP_MIN_FILTER_PRICE').' '.$name,
						'PREFIX' => Loc::getMessage('BT_MOD_CATALOG_COND_CMP_MIN_FILTER_PRICE_PREFIX').' '.$name,
						'LOGIC' => static::GetLogic(array(BT_COND_LOGIC_EQ/*, BT_COND_LOGIC_NOT_EQ, BT_COND_LOGIC_GR, BT_COND_LOGIC_LS, BT_COND_LOGIC_EGR, BT_COND_LOGIC_ELS*/)),
						'JS_VALUE' => array(
							'type' => 'input'
						),
...
class SMCatalogCondCtrlIBlockProps extends SMCatalogCondCtrlComplex
{
	/**
	 * @param bool|string $strControlID
	 * @return bool|array
	 */
	public static function GetControls($strControlID = false, $iblockId = null)
	{
		$arControlList = array();
		$arIBlockList = array();
		...
		//$rsIBlocks = CCatalog::GetList(array(), $arFilter, false, false, array('IBLOCK_ID', 'PRODUCT_IBLOCK_ID'));                                        //////////////////////////////////////////////////////////////
		$rsIBlocks = CIBlockElement::GetList(array(), $arFilter, array('IBLOCK_ID', 'PRODUCT_IBLOCK_ID'), false, array('IBLOCK_ID', 'PRODUCT_IBLOCK_ID'));
				...
				if (false !== $strName)
				{
					...
					while ($arProp = $rsProps->Fetch())
					{
						...
						elseif($arProp['PROPERTY_TYPE'] == 'S' && $arProp['USER_TYPE']=='directory'){
							...
							$arProp['VALUES'] = array();
							$res = $entity_data_class::getList(array('order'=>array('ID' => 'ASC')));
							while($one = $res->fetch()){
								$arProp['VALUES'][] = $one;
							}
							if (empty($arProp['VALUES']))
							...
							switch ($arProp['USER_TYPE'])
							{
								...
								case 'directory':
									...
									foreach ($arProp['VALUES'] as &$arOnePropValue)
									{
										...
										$arValue['values'][$arOnePropValue['ID']] = isset($arOnePropValue['UF_NAME'])?$arOnePropValue['UF_NAME']:'id#'.$arOnePropValue['ID'];
									}
									$boolUserType = true;
									break;
								default:
						...
						$arControlList['CondIBProp:'.$intIBlockID.':'.$arProp['ID']] = array(
							...
							'LABEL' => $arProp['NAME']." [$arProp[ID]]",
							'PREFIX' => str_replace(array('#NAME#', '#IBLOCK_ID#', '#IBLOCK_NAME#'), array($arProp['NAME']." [$arProp[ID]]", $intIBlockID, $strName), Loc::getMessage('BT_MOD_CATALOG_COND_CMP_CATALOG_ONE_PROP_PREFIX')),
							'LOGIC' => $arLogic,
							'JS_VALUE' => $arValue,
						...
						if($arProp['PROPERTY_TYPE'] == 'N'){
							$arControlList['CondIBMaxFilterProperty:'.$intIBlockID.':'.$arProp['ID']] = array(
								'ID' => 'CondIBMaxFilterProperty:'.$intIBlockID.':'.$arProp['ID'],
								'FIELD' => 'MaxFilterProperty'.$arProp['ID'],
								'FIELD_TYPE' => 'int',
								'LABEL' => Loc::getMessage('BT_MOD_CATALOG_COND_CMP_MAX_FILTER_PRICE').' '.$arProp['NAME']." [$arProp[ID]]",
								'PREFIX' => Loc::getMessage('BT_MOD_CATALOG_COND_CMP_MAX_FILTER_PRICE_PREFIX').' '.$arProp['NAME'],
								'LOGIC' => static::GetLogic(array(BT_COND_LOGIC_EQ/*, BT_COND_LOGIC_NOT_EQ, BT_COND_LOGIC_GR, BT_COND_LOGIC_LS, BT_COND_LOGIC_EGR, BT_COND_LOGIC_ELS*/)),
								'JS_VALUE' => array(
									'type' => 'input'
								),
							...
							$arControlList['CondIBMinFilterProperty:'.$intIBlockID.':'.$arProp['ID']] = array(
								'ID' => 'CondIBMinFilterProperty:'.$intIBlockID.':'.$arProp['ID'],
								'FIELD' => 'MinFilterProperty'.$arProp['ID'],
								'FIELD_TYPE' => 'int',
								'LABEL' => Loc::getMessage('BT_MOD_CATALOG_COND_CMP_MIN_FILTER_PRICE').' '.$arProp['NAME']." [$arProp[ID]]",
								'PREFIX' => Loc::getMessage('BT_MOD_CATALOG_COND_CMP_MIN_FILTER_PRICE_PREFIX').' '.$arProp['NAME'],
								'LOGIC' => static::GetLogic(array(BT_COND_LOGIC_EQ/*, BT_COND_LOGIC_NOT_EQ, BT_COND_LOGIC_GR, BT_COND_LOGIC_LS, BT_COND_LOGIC_EGR, BT_COND_LOGIC_ELS*/)),
								'JS_VALUE' => array(
									'type' => 'input'
								),
	...
	public static function GetControlShow($arParams, $iblockId = null)
	{
		$arControls = static::GetControls(false, $iblockId);
		$arResult = array();
		$intCount = -1;
		foreach ($arControls as &$arOneControl)
		{
...
class SMGlobalCondTree
{
	...
	{
		CJSCore::Init(array("sotbit_condtree"));
	}
		...
		$arEventList = array(
			BT_COND_BUILD_CATALOG => array(
				...
				'CONTROLS' => array(
					...
					'EVENT_ID' => 'OnCondCatControlBuildListSM'
				)
			),
	...
	public function Show($arConditions, $iblock_id = null)
	{
		$this->arMsg = array();
		if (!$this->boolError)
		{
		...
		if (!$this->boolError)
		{
			...
			$strResult .= $this->ShowControls($iblock_id)."\n";
	...
	public function Parse($arData = '', $arParams = false)
	{
		...
		if (!$this->boolError)
		{
			foreach ($arData as $strKey => $value)
			{
				...
				{
					$arOneCondition=array('logic' => $equal,'value'=>'');
					/*$this->boolError = true;
	...
	public function ShowControls($iblock_id = null)
	{
		if ($this->boolError)
			return '';
...
class SMCondTree extends SMGlobalCondTree
{
	public function __construct()
	{
