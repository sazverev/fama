...
class CSeoMeta extends Bitrix\Iblock\Template\Functions\FunctionBase {
    ...
    function GetValuesForEnum_NoCache(
        $enum_fields = false,
        $Xvalue = []
    ) {
        $ListValues = [];
            ...
            if($idCondition == '' && isset($FilterResult['ITEMS'][$idSection]['VALUES']) && is_array($FilterResult['ITEMS'][$idSection]['VALUES'])) {
                ...
                foreach($FilterResult['ITEMS'][$idSection]['VALUES'] as $key => $param) {
                    if(
                        ...
                        || strcmp($param['VALUE'], $idCondition) == 0
    ...
    private function CheckElementsNot(
        $idSection,
        $idCondition,
        $type = 0
    ) {
    ...
    private function CheckElementsContain(
        $idSection,
        $idCondition,
        $type = 0
    ) {
    ...
    private function CheckElementsNotCont(
        $idSection,
        $idCondition,
        $type = 0
    ) {
    ...
    private function CheckElementsGreat(
        $idSection,
        $idCondition,
        $type = 0
    ) {
    ...
    private function CheckElementsLess(
        $idSection,
        $idCondition,
        $type = 0
    ) {
    ...
    private function CheckElementsEqGr(
        $idSection,
        $idCondition,
        $type = 0
    ) {
    ...
    private function CheckElementsEqLs(
        $idSection,
        $idCondition,
        $type = 0
    ) {
    ...
    private function ANDFalseConditions(
        $conditions
    ) {
        $return = 0;
    ...
    private function ORFalseConditions(
        $conditions
    ) {
        $return = 0;
        foreach ($conditions as $key => $condition) {
            ...
            if(count($NeedFields) > 0) {
                ...
                if($res = $ar_result->GetNext()) {
                    foreach($NeedFields as $NeedField) {
                        self::$UserFields['#'.$NeedField.'#'] = $res[$NeedField] ?: '#'.$NeedField.'#';
                    }
                }
            }
        }
