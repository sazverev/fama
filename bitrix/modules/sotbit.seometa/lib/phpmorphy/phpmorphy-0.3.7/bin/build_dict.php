...
class ShellArgsEscaper {
    ...
    static function escape($arg) {
        ...
        if(self::$need_wrap) {
            ...
            $orig_len = mb_strlen($arg);
            $slashes = $orig_len - mb_strlen(rtrim($arg, '\\'));
    ...
    while($reader->read()) {
        if($reader->nodeType == XMLReader::ELEMENT) {
            if($reader->localName === 'locale') {
                ...
                $result = mb_strlen($result) ? $result : false;
                break;
            }
        }
    }
...
$out_dir = $argv[2];
$morph_data_file = $out_dir . '/morph_data.' . mb_strtolower($locale) . '.bin';
