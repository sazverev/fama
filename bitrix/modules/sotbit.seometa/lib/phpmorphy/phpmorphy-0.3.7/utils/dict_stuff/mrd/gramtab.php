...
class phpMorphy_GramTab_GramInfo_Russian extends phpMorphy_GramTab_GramInfo {
    ...
    private function processPos() {
        if($this->pos == 'Ã') {
            if(in_array('ïð÷', $this->grammems)) {
                $this->pos = 'ÏÐÈ×ÀÑÒÈÅ';
            } elseif(in_array('äïð', $this->grammems)) {
                $this->pos = 'ÄÅÅÏÐÈ×ÀÑÒÈÅ';
            } elseif(in_array('èíô', $this->grammems)) {
                $this->pos = 'ÈÍÔÈÍÈÒÈÂ';
            }
        }
    }
...
class phpMorphy_GramTab_WithoutComments_Iterator extends phpMorphy_Iterator_NotEmpty {
    protected function isEmptyItem($item) {
        ...
        if(!mb_strlen($item)) return true;
        if(mb_substr($item, 0, 2) == '//') return true;
