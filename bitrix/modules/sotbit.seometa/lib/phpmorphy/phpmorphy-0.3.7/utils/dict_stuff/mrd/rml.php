...
class phpMorphy_Rml_IniFile {
	...
	protected function parseFile($file) {
		...
		foreach($lines as $line) {
			if(false !== ($pos = mb_strpos($line, ' ')) || false !== ($pos = mb_strpos($line, "\t"))) {
				$key = trim(mb_substr($line, 0, $pos));
				$value = $this->replaceRmlVar(trim(mb_substr($line, $pos + 1)));
