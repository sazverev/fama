...
class phpMorphy_Mrd_Section_Flexias extends phpMorphy_Mrd_Section {
	...
	protected function processLine($line) {
		...
		foreach(explode('%', mb_substr($line, 1)) as $token) {
	...
	protected function removeComment($line) {
		if(false !== ($pos = mb_strrpos($line, self::COMMENT_STRING))) {
			return rtrim(mb_substr($line, 0, $pos));
		} else {
			return $line;
		}
	}
...
class phpMorphy_Mrd_File {
	...
	protected function getSectionClassName($sectionName) {
		return 'phpMorphy_Mrd_Section_' . ucfirst(mb_strtolower($sectionName));
	}
