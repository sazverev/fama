...
BX.TreeCondCtrlBaseSelect.prototype.setValueText = function () {
    ...
    if (this.id === 'value' && this.type === 'select') {
        const properties = this.select.querySelectorAll('.custom-value-select__property');

        for (i = 0; i < properties.length; i++) {
            if (BX.util.in_array(properties[i].dataset.value, this.valuesContainer[this.id])) {
                j = BX.util.array_search(properties[i].dataset.value, this.values);
                if (j > -1) {

                    const resultInput = document.getElementById(this.parentContainer.id + '_' + this.id);
                    const valueInput = document.getElementById(this.parentContainer.id + '_' + this.id + '_send');

                    valueInput.value = properties[i].dataset.value;

                    if (resultInput.dataset.index === "-1") {
                        resultInput.dataset.index = i;
                        resultInput.value = properties[i].textContent;
                    }
    ...
    BX.TreeCondCtrlBaseSelect.prototype.SetValue = function () {
        ...
        if (this.multiple) {
            ...
            if (this.id === 'value' && this.type === 'select') {
                const resultInput = document.getElementById(this.parentContainer.id + '_' + this.id);
                const options = this.select.querySelectorAll('.custom-value-select__property');
                const valueInput = document.getElementById(this.parentContainer.id + '_' + this.id + '_send');

                if (resultInput.dataset.index > -1 && options[resultInput.dataset.index]) {
                    this.valuesContainer[this.id] = [valueInput.value];
                    i = BX.util.array_search(valueInput.value, this.values);
...
BX.TreeCondCtrlBaseSelect.prototype.CreateSelect = function() {
    ...
    if (this.id === 'value' && this.type === 'select') {
        ...
        this.select.appendChild(BX.create(
            'INPUT',
            {
                props: {
                    id: this.parentContainer.id + '_' + this.id,
                    type: 'text',
                    placeholder: this.first_option,
                    className: 'custom-value-select__result-input',
                },
                attrs: {
                    readonly: 'true'
                },
        ...
        this.select.appendChild(BX.create(
            'INPUT',
            {
                props: {
                    type: 'hidden',
                    name: this.name,
                    id: this.parentContainer.id + '_' + this.id + '_send'
                },
            }
        ));

        this.selectWrapper = this.select.appendChild(BX.create(
            'DIV',
            {
                props: {
                    className: 'custom-value-select__props-wrapper'
                }
            }
        ));

        this.selectWrapper.appendChild(BX.create(
            'INPUT',
            {
                props: {
                    type: 'text',
                    className: 'custom-value-select__search-props'
                },
                events: {
                    input: function () {
                        const itemsContainer = this.parentNode.querySelectorAll('.custom-value-select__property');
                        for (let i = 0; i < itemsContainer.length; i++) {
                            if (!itemsContainer[i]
                                ...
                                itemsContainer[i].style.display = 'block';
                            }
                        }
                    }
                }
        ...
        this.selectPropsContainer = this.selectWrapper.appendChild(BX.create(
            'DIV',
            {
                props: {
                    className: 'custom-value-select__props-container'
                }
            }
        ));
        ...
        if (BX.type.isElementNode(this.select)) {
            if (!this.multiple && !this.dontShowFirstOption) {
                this.selectPropsContainer.appendChild(BX.create(
                    'div',
                    {
                        props: {
                            className: 'custom-value-select__property'
                        },
                        dataset: {
                            value: '',
                            index: 0
                        ...
                        events: {
                            click: BX.delegate(
                                function () {
                                    const resultInput = document.getElementById(this.parentContainer.id + '_' + this.id);
                                    const sendInput = document.getElementById(this.parentContainer.id + '_' + this.id + '_send');

                                    sendInput.value = '';
                                    resultInput.value = this.first_option;
                                    resultInput.parentNode.classList.remove('active');
                                    resultInput.dataset.index = '0';
                                    this.onChange();
                                },
                                this)
                        },
			...
			for (i in this.values) {
                ...
                if (this.values.hasOwnProperty(i)) {
                    if (this.id === 'value' && this.type === 'select') {
                        ...
                        const propertyTextContent = BX.util.htmlspecialchars(this.ViewFormat(this.values[i], this.labels[i]));
                        const index = a + 1;
                                ...
                                events: {
                                    click: BX.delegate(
                                        function () {
                                            ...
                                            resultInput.value = propertyTextContent;
                                            resultInput.parentNode.classList.remove('active');
                                            resultInput.dataset.index = a.toString();
                                            this.onChange();
                                        },
                                        this)
                                },
	...
	if (this.boolResult)
	{
		if (!!obTreeLevel.container)
		{
			if (CurControl.group)
			{
                ...
                const addCustomSelect = divAdd.appendChild(BX.create(
                    'DIV',
                    {
                        props: {
                ...
                addCustomSelect.appendChild(BX.create(
                    'INPUT',
                    {
                        props: {
                            className: 'custom-select__input',
                            type: 'text',
                            placeholder: (!!CurControl.mess && !!CurControl.mess.SELECT_CONTROL ? CurControl.mess.SELECT_CONTROL : this.messTree.SELECT_CONTROL)
                        },
                        attrs: {
                            readonly: 'true'
                        },
                ...
                const propsWrapper = addCustomSelect.appendChild(BX.create(
                    'DIV',
                    {
                        props: {
                            className: 'custom-select__props-wrapper'
                        },
                    }
                ));

                const searchProps = propsWrapper.appendChild(BX.create(
                    'input',
                    {
                        props: {
                            className: 'custom-select__search-props',
                            type: 'text'
                        },
                        events: {
                            input: function () {
                                const itemsContainer = this.parentNode.querySelectorAll('.custom-select__item');
                                const groups = this.parentNode.querySelectorAll('.custom-select__group');
                                ...
                                Array.from(groups).forEach(
                                    function (group) {
                                        const groupItems = group.querySelectorAll('.custom-select__item');

                                        group.style.display = allItemsAreHidden(groupItems)
                                            ? 'none'
                                            : 'block';
                                    }
                                );

                                function allItemsAreHidden(items) {
                                    let result = true;
                ...
                const propsContainer = propsWrapper.appendChild(BX.create(
                    'DIV',
                    {
                        props: {
                            className: 'custom-select__props-container'
                        },
                    }
                ));
                        ...
                        if (BX.util.in_array(CurControl.controlId, this.controls[i].showIn)) {
                            if (!!this.controls[i].controlgroup) {
                                ...
                                grp = BX.create(
                                    ...
                                    {
                                        props: {
                                            className: 'custom-select__group'
                                        },
                                        html: '<span class="custom-select__title">' + this.controls[i].label + '</span>'
                                    }
                                );

                                const propsGroupContainer = BX.create(
                                    'DIV',
                                    {
                                        props: {
                                            className: 'custom-select__group-container'
                                        }
                                    }
                                );
                                ...
                                if (!!grp && !!this.controls[i].children && !!this.controls[i].children.length && this.controls[i].children.length > 0) {
                                    for (j = 0; j < this.controls[i].children.length; j++) {
                                        if (BX.util.in_array(CurControl.controlId, this.controls[i].children[j].showIn)) {
                                            found = true;
                                            const value = this.controls[i].children[j].controlId;

                                            propsGroupContainer.appendChild(BX.create(
                                                'DIV',
                                                {
                                                    props: {
                                                    ...
                                                    events: {
                                                        click: BX.delegate(
                                                            function () {
                                                                const customSelect = document.querySelector('[data-id=' + divAdd.id + '_select]');
                                                                customSelect.classList.remove('active');
                                                                customSelect
                                                                    .querySelector('.custom-select__search-props')
                                                                    .value = '';
                                                                Array.from(customSelect.querySelectorAll('.custom-select__item'))
                                                                    .forEach(
                                                                        function (item) {
                                                                            item.style.display = 'block';
                                                                        }
                                                                    );
                                                                Array.from(customSelect.querySelectorAll('.custom-select__group'))
                                                                this.CreateLevel(obTreeLevel, value);
                                                                BX.style(addCustomSelect, 'display', 'none');
                                                                BX.style(addBtn, 'display', '');
                                                            },
                                                            this
                                                        )
                                                    },
                                    ...
                                    if (found) {
                                        grp.appendChild(propsGroupContainer);
                                        propsContainer.appendChild(grp);
                                    }
                                }
                            } else {
                                const value = this.controls[i].controlId;

                                propsContainer.appendChild(BX.create(
                                    'DIV',
                                    {
                                        props: {
                                            className: 'custom-select__item'
				...
				if (!!addBtn && !!addCustomSelect)
				{
					divAdd.link = addBtn;
					divAdd.customSelect = addCustomSelect;

					BX.bind(addBtn,'click', BX.delegate(
                        function () {
                            BX.style(divAdd.customSelect, 'display', '');
                            BX.style(divAdd.link, 'display', 'none');
                            BX.focus(divAdd.customSelect);
                        }, divAdd
                    ));
                    ...
                    BX.bind(addSelect, 'change', BX.delegate(
                        function () {
                            ...
                            BX.style(divAdd.customSelect, 'display', 'none');
                            BX.style(divAdd.link, 'display', '');
                        }, this
                    ));

                    BX.bind(addCustomSelect, 'blur', BX.delegate(
                        function () {
                            divAdd.select.selectedIndex = 0;
                            BX.style(divAdd.customSelect, 'display', 'none');
                            BX.style(divAdd.link, 'display', '');
...
function closeCustomSelects() {
    document.addEventListener('click', function (event) {
        ...
        && !node.classList.contains('custom-select')
        && !node.classList.contains('custom-value-select')) {
            node = node.parentNode;
        }

        if (node) {
            if (!node.classList.contains('custom-select') && document.querySelector('.custom-select')) {
                document.querySelector('.custom-select').classList.remove('active');
            }

            if (!node.classList.contains('custom-value-select') && document.querySelector('.custom-value-select')) {
                Array.from(document.querySelectorAll('.custom-value-select')).forEach(
                    function (item) {
...
function searchMetaProps() {
    const searchPropsInputs = document.querySelectorAll('.metainform__search-input');

    Array.from(searchPropsInputs).forEach(
        function (input) {
            input.addEventListener('input', function () {
                const props = input.parentNode.parentNode.querySelectorAll('.with-prop');
