...
<div class=\"ipol_subFaq\">
    ...
    <div class=\"ipol_inst\">
        <ul>
            ...
            <li>Стоимость доставки несколько различается для нового и старого API ввиду неучета НДС в API 2.0.</li>
...
$MESS['IPOLSDEK_OPT_addDeparture'] = "Дополнительные города-отправители <a href='#' class='PropHint' onclick='return IPOLSDEK_setups.popup(\"pop-addDeparture\", this);'></a>";
    // Отправитель
$MESS['IPOLSDEK_OPT_sender_company'] = "Наименование компании отправителя";
$MESS['IPOLSDEK_OPT_sender_name']    = "Контактное лицо отправителя";
$MESS['IPOLSDEK_OPT_sender_address'] = "Адрес отправителя";
$MESS['IPOLSDEK_OPT_sender_street']  = "улица";
$MESS['IPOLSDEK_OPT_sender_house']   = "дом";
$MESS['IPOLSDEK_OPT_sender_flat']    = "офис";
$MESS['IPOLSDEK_OPT_sender_phone']   = "Телефон отправителя";
    // Печать
...
$MESS ['IPOLSDEK_HELPER_addDeparture'] = "Используются для изменения города-отправителя в форме оформления заявки.";
		// Данные отправителя
$MESS ['IPOLSDEK_HELPER_sender_phone'] = "Номер телефона рекомендуется передавать в следующем формате:
    <ul><li>номер должен быть написан в международном формате: код страны (для России +7) и сам номер (10 и более цифр);</li>
        <li>номер не должен содержать никаких букв и символов, кроме специальных символов:
        \",\" (запятая), \";\" (точка с запятой), \"/\" (косая черта), которые служат разделителями между номерами</li>
        <li>\":\" (двоеточие), \"#\" (решетка), которые являются признаком добавочного номера</li></ul>";
        // Печать
...
$MESS ['IPOLSDEK_HDR_sender'] = "Данные отправителя";
...
$MESS ['IPOLSDEK_FAQ_DEAD_SERVER_DESCR'] = "<p>Ждать, пока техническая поддержка СДЭК восстановит работу сервера. К сожалению, с серверами СДЭКа иногда возникают технические проблемы, сервера \"падают\".<br>Писать в техподдержку модуля нерационально: у нас нет доступов к серверам СДЭК, мы никак не можем повлиять на эту ситуацию.<br>Кнопка \"Сброс\" предназначена для сброса времени ожидания подъема сервера. Его работоспособность не может восстановиться моментально - чтобы не задерживать оформление заказа, модуль ждет определенное время, указанное в настройке \"Ожидание подъема сервера\" в сервисных свойствах.<p>";
    // Отправитель
$MESS['IPOLSDEK_FAQ_sender_TITLE'] = "- Зачем нужна информация об отправителе";
$MESS['IPOLSDEK_FAQ_sender_DESCR'] = "<p>Данная группа настроек служит для установки данных по умолчанию, которые будут всегда заполняться в заявке, а также - браться в автоотгрузках. Вы всегда можете отредактировать данные в заявке (блок откроется по щелчку).</p>
<p>Информация по отправителю может быть необходима в двух случаях:<ul><li>Информация отличается от выставленной в Личном кабинете;</li><li>Заказы сдаются на складе ПВЗ, и он требует указывать свой адрес.</li></ul>Имейте в виду: данные будут полностью перезаписаны!</p>
<p>При передаче данных об отправителе телефон является обязательным полем.</p>
<div class=\"ipol_subFaq\">
        <a class=\"ipol_smallHeader\" onclick=\"$(this).next().toggle(); return false;\">&gt; О загрузке данных в накладную и ЛК.</a>
        <div class=\"ipol_inst\">
        Когда речь идет об установке информации о заказе в поле - подразумевается блок Отправителя.
        <ul>
            <li>\"Наименование компании отправителя\" заменяет информацию в блоке Отправитель-Компания накладной и устанавливается в информацию о заказе в поле \"Отправитель\".</li>
            <li>\"Истинный продавец\" заменяет информацию в блоке Отправитель-Продавец накладной.</li>
            <li>\"Контактное лицо отправителя\" устанавливается в информацию о заказе в поле \"Имя менеджера\".</li>
            <li>\"Адрес отправителя\" устанавливается в информацию о заказе в поле \"Адрес\".</li>
            <li>\"Телефон отправителя\" устанавливается в информацию о заказе в поле \"Телефон\".</li>
        </div>
        Если вы используете функционал отправителей - данные не перезапишутся, так как этот блок отвечает за данные отправителя заказа, а функционал отправителей - за забор заказа.";
