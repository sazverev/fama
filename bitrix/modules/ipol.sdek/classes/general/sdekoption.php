		...
		static function getOrderStates(){//запрос статусов заказов
            ...
            if($bdOrders->Fetch()){
                ...
                foreach($accounts as $id => $acc){
                    // Check all orders for this account except orders in final statuses
                    $ordersStr = '';
                    $requests = sqlSdekOrders::select(array(), array('ACCOUNT' => $id, 'OK' => true, 'STATUS' => array('OK', 'DELETE', 'STORE', 'TRANZT', 'CORIER', 'PVZ')));
                    while ($tmp = $requests->Fetch())
                    {
                        $ordersStr .= '<Order DispatchNumber="'.$tmp['SDEK_ID'].'"/>';
                    }
