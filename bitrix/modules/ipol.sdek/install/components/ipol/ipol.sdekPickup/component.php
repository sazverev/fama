...
if(!(count($arParams['CITIES'])==1 && $arResult['DELIVERY']['pickup'] == 'no')){
    ...
    if(CDeliverySDEK::$goods)
    {
        $arGabs = CDeliverySDEK::$goods;
        $arGabs['W'] *= 1000;
    }
    else
    {
        $arGabs = array('W' => (CDeliverySDEK::$orderWeight) ? false : \Ipolh\SDEK\option::get('weightD'));
    }

	$arList['POSTAMAT'] = CDeliverySDEK::weightPST($arGabs,$arList['POSTAMAT']);
}
