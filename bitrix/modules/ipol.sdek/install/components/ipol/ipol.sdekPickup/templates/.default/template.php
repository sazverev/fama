...
$mode = (!array_key_exists('MODE',$arParams) || $arParams['MODE'] == 'both') ? array('PVZ','POSTAMAT') : array($arParams['MODE']); // Profiler

if($arParams['CNT_DELIV'] == 'Y'){
    $prices = array();
    foreach(array('courier','pickup','postamat') as $profile)// Profiler
    ...
    var IPOLSDEK_pvz = {
        ...
        initCityPVZ: function(){ // loading PVZ 4 chosen city
            ...
            if(IPOLSDEK_pvz.punctMode !== 'PVZ'){
                for(var i in IPOLSDEK_pvz.POSTAMAT[city]){
                    IPOLSDEK_pvz.cityPVZ[i] = {
                        'Name'     : (IPOLSDEK_pvz.POSTAMAT[city][i]['Name']) ? IPOLSDEK_pvz.POSTAMAT[city][i]['Name'] : IPOLSDEK_pvz.POSTAMAT[city][i]['Address'],
                        'Address'  : IPOLSDEK_pvz.POSTAMAT[city][i]['Address'],
                        'WorkTime' : IPOLSDEK_pvz.POSTAMAT[city][i]['WorkTime'],
                        'Phone'    : IPOLSDEK_pvz.POSTAMAT[city][i]['Phone'],
                        'Note'     : IPOLSDEK_pvz.POSTAMAT[city][i]['Note'],
                        'cX'       : IPOLSDEK_pvz.POSTAMAT[city][i]['cX'],
                        'cY'       : IPOLSDEK_pvz.POSTAMAT[city][i]['cY'],
                        'type'	   : 'POSTAMAT'
                    };
                }
            }
...
//console.log('onLoadPrices',data);
        ...
        setPrices: function(){
            ...
            if(IPOLSDEK_pvz.punctMode !== 'ALL'){ // Profiler
                ...
                $('#SDEK_iPrice').html(IPOLSDEK_pvz.prices.postamat[0]);
                $('#SDEK_iDate').html(IPOLSDEK_pvz.prices.postamat[1]);

            }else {
                $('.IPOLSDEK_subPunct_detail_PVZ').each(function () {
                    $(this).html(IPOLSDEK_pvz.prices.pickup[0] + " &nbsp;" + IPOLSDEK_pvz.prices.pickup[1]);
                });
                $('.IPOLSDEK_subPunct_detail_POSTAMAT').each(function(){
                    $(this).html(IPOLSDEK_pvz.prices.postamat[0]+" &nbsp;"+IPOLSDEK_pvz.prices.postamat[1]);
                });
            }
        },
