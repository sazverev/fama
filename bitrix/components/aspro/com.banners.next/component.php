...
if($this->StartResultCache(false, array(($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $bUSER_HAVE_ACCESS, $arNavigation, $arrFilter, $BIGBANNER_MOBILE)))
{
	...
	if($arResult = $rsIBlock->GetNext())
	{
		...
		if($arParams["BANNER_TYPE_THEME"])
		{
			...
			$rsItem = CIBlockElement::GetList(Array("SORT"=>"ASC"),  Array("IBLOCK_ID" => $arParams["TYPE_BANNERS_IBLOCK_ID"], "CODE" => $arCode), false, false, Array("IBLOCK_ID", "ID", "CODE"));
			while($arItem = $rsItem->Fetch())
				$arBannersCode[] = $arItem;
		}
		...
		foreach($arBannersCode as $key => $arTypeBaner)
		{
			...
			while($obElement = $rsElement->GetNextElement())
			{
				...
				$arItem["TYPE_BANNER"] = $arTypeBaner["CODE"];
