...
if ($cache->initCache($cacheTime, $cache_id, $cacheDir)) {
    ...
    $currentUrl = Encoding::convertEncoding($currentUrl, LANG_CHARSET, "utf-8", $error);
    preg_match_all('/[\p{Cyrillic}]+/iu', $currentUrl, $match);
    foreach ($match[0] as $i => $m) {
        $currentUrl = str_replace($m, rawurlencode($m), $currentUrl);
    }
